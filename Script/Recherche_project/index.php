<link rel="stylesheet" href="Script/Recherche_project/style.css" type="text/css"> 
    <?php 
        $bdd = new PDO('mysql:host=localhost;dbname=anthfpxc_web;charset=utf8', 'anthfpxc', 'rItxfJ96O9cE');
        //$bdd = new PDO('mysql:host=localhost;dbname=my_site;charset=utf8', 'root', '');
        if (!empty($_POST['recherche']))
        {
            $search = $_POST['recherche'];
            $reponse = $bdd->query("SELECT * FROM projects WHERE Titre LIKE '%" . $search . "%' OR Presentation LIKE '%" . $search . "%'");
            $Msg = "Projet qui comporte : \" " . $search . " \"";
            echo '<h4 class="desc_cat">'. $Msg . '</h4>';
        }
        else 
        {
